...
**Extra (`extra_enabled`, `../_shared/extra-dispatch.md`):** the same slice may run off Claude. No score exists here either, so resolve the pinned executor's **BAND, both edges, agreeing** — otherwise the run stays on Claude. The `extra:` line joins the F0 preflight (P0, printed whenever the gate is on); dispatch through `orc extra dispatch --task <file> --json`; validate with `return-validation.md` **§2b, not §2**; a failure falls back to the pinned agent, announced. Extra is orthogonal to the knowledge gate — a fresh wiki and a cached pattern are still required, and a foreign worker gets the same pointers-not-content slice.
