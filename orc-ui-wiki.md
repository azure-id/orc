...
| **v1.9.0 widened both lean lanes and this panel did not change** — `/orc-quick` gained `graph-map`, `graph-coverage`, `graph-changes` and `graph-gain`, `/orc-mini` gained `graph-impact`, `graph-cochange` and `graph-gain`, `graph-notes-pending` LOST both of them, and `orc-quick`'s `q3` phase gained five trace verbs. **Not one line of `js/panels/lanes.js` moved.** That is the derivation rule paying for itself: the panel reads `lane calls --all` and `lane phases <lane>` and owns no id, so a catalogue change reaches the UI the moment the CLI answers differently. A release that DID need a panel edit here would be the drift signal, not the feature | nothing in `bin/webui/`; the change is `LANE_CALLS` + `LANE_OWN_PHASES` in `bin/cli.js`, asserted by `test/cli/graph-lanes.test.js` |
